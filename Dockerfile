...
RUN mkdir /var/run/sshd && echo 'root:shibby' | chpasswd && \
...
RUN apt-get install -y net-tools vim ctags
...
RUN useradd -ms /bin/bash tomato && echo 'tomato:shibby' | chpasswd && adduser tomato sudo
